...
@interface YetTakeGoodsController ()

@end

@implementation YetTakeGoodsController
...
//    _topView.backgroundColor=[UIColor colorWithRed:0.51 green:0.57 blue:0.29 alpha:1];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notifirefresh) name:@"commentsuccess" object:nil];
...
//                [Dialog simpleToast:@"暂无信息"];
    ...
    [dataProvider getOrderList:@"40" andpage:page andPerPage:perpage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *CellIdentifier = @"yetTakeCellIdentifier";
    ...
    [cell.btnSureTake setTitle:@"退货申请" forState:UIControlStateNormal];
    ...
    if([[[[[arrayWaitpay objectAtIndex:indexPath.section] objectForKey:@"order_list"] objectAtIndex:indexPath.row] objectForKey:@"evaluation_state"] isEqualToString:@"0"]){
            [cell.btnSearchWuLiu setTitle:@"评价商品" forState:UIControlStateNormal];
    }else{
            [cell.btnSearchWuLiu setTitle:@"已评价" forState:UIControlStateNormal];
    }
...
//    [cell.ImgGoods setImageWithURL:url placeholderImage:img(@"icon_default.png")];
//    
//    
...
//    [btnpay setTitle:@"退货申请" forState:UIControlStateNormal];
//    btnpay.titleLabel.font=[UIFont systemFontOfSize:13.0] ;
//    btnpay.tag=2001+section;
//    [btnpay setTitleColor:[UIColor orangeColor] forState:UIControlStateNormal];
//        btnpay.layer.borderWidth=0.6;
//        btnpay.layer.borderColor=[[UIColor orangeColor]CGColor];
...
//    NSLog(@"***%ld***%ld****%d",(long)path.section,(long)path.row,sender.tag-700);
    ...
    GoodDetail.goodsId=[[[[[[arrayWaitpay objectAtIndex:path.section] objectForKey:@"order_list"] objectAtIndex:path.row] objectForKey:@"order_goods"] objectAtIndex:sender.tag-753] objectForKey:@"goods_id"];
...
-(void)suretake:(UIButton *)sender
{
    ...
    ReturnMoneyController *commentedit=[[ReturnMoneyController alloc]init];
    commentedit.orderId=[[[[arrayWaitpay objectAtIndex:path.section] objectForKey:@"order_list"] objectAtIndex:path.row] objectForKey:@"order_id"];
    
    [self.navigationController pushViewController:commentedit animated:YES];
...
-(void)searchwuliu:(UIButton *)sender
{
    ...
    if([[[[[arrayWaitpay objectAtIndex:path.section] objectForKey:@"order_list"] objectAtIndex:path.row] objectForKey:@"evaluation_state"] isEqualToString:@"0"]){
        CommentEditController *commentedit=[[CommentEditController alloc]init];
        commentedit.orderId=[[[[arrayWaitpay objectAtIndex:path.section] objectForKey:@"order_list"] objectAtIndex:path.row] objectForKey:@"order_id"];
        
        [self.navigationController pushViewController:commentedit animated:YES];
    }else{
        [SVProgressHUD showErrorWithStatus:@"亲，商品已经评价过了哟！"];
    }
